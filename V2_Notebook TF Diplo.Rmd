...
doc_stm_8 %>%
  ...
  mutate(id = as.integer(id)) %>%
  left_join(corpus_base %>% select(id, medio) %>% unique()) %>%
   na.omit(medio) %>% # Revisar, algunos tópicos quedan asociados a Medios NA
  group_by(medio, topic) %>%
    summarise(mean = mean(gamma)*100) %>%
  ggplot() +
    geom_col(aes(x=topic, y=mean, fill=medio), position='dodge') +
...
ggplot(combined_data, aes(x = medio, y = mean, fill = model, label = topic)) +
  ...
  geom_text(position = position_dodge(width = 1),    # Posición de las etiquetas
            #vjust = -0.5,                             # Alineación vertical
            size = 3,                                 # Tamaño de la letra
            aes(color = model),                       # Color de las etiquetas
            angle = 0,                               # Ángulo de las etiquetas
  ...
  labs(title = "Predominancia del tópico más relevante por modelo y medio",
       ...
       fill = "Modelo") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_manual(values = c("LDA" = "grey", "STM" = "grey3")) +
  scale_color_manual(values = c("LDA" = "black", "STM" = "white"))  # Color de las etiquetas

```
Me falta acomodar las etiquetas del gráfico y los colores así se entiende. La idea es mostrar que tópico le asigna como predominante cada modelo a los medios. 
------------------------------------------------------------------------

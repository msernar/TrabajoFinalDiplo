...
Vamos a generar dos matrices parea explorar los resultados del modelo: palabras por tópico y documentos por tópico
...
doc_stm_5 <- tidy(stm_5, matrix = 'theta')
```
...
A modo de prueba, y considerando que STM pudo identificar un nuevo tópico, corremos el módelo con un k mayor.
```{r}
...
# matriz de documentos por tópicos
doc_stm_10 <- tidy(stm_10, matrix = 'theta')
 ...
 terminos_stm_10 %>%
  ...
  theme_minimal()
```
Repite política interncional, pero suma un tópico relacionado a medicina.
------------------------------------------------------------------------

...
COMENTARIO: EN REALIDAD, ESAS NUEVAS "STOPWORDS" NO TENDRÍAN MÁS QUE VER CON EL HECHO DE QUE SON TÉRMINOS MUY ESPECÍFICOS QUE NO FIGURAN EN LOS DICCIONARIOS QUE SE USAN PARA LIMPIAR LAS STOPWORDS? 
--> si, es eso! en clase dijeron que muchas veces hay que ajustar los listados de stopwords al corpus que estemos trabajando

Para ampliar el listado de stopwords partimos del listado de palabras con sus métricas de tf e idf.

COMENTARIO: ACÁ ME PERDÍ UN POCO: ESTA LIMPIEZA DE PUNTOS NO ESTABA HECHA EN LOS PRIMEROS PASOS DEL TP?
--> el código del inicio transforma letras con acentos o diéresis en letras normales, no elimina puntos que esten entre letras
...
### A PARTIR DE ACÁ
intenté calcular las métricas por noticia, pero como la consigna dice "términos más importantes por medio" me pareció incorrecto. Volví al original de conteo por medio.
...
```{r LDA_v2}
lda_5_v2 <- LDA(disc_dtm_2, k = 5, control = list(seed = 1234))  # k es el número de tópicos a identificar

ap_topics_2 <- tidy(lda_5_v2, matrix = "beta") 
# la función tidy convierte el modelo a un tópico-término por fila
